...
export class CreateDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.consumerForm = this._consumerFormBuilder.group({
      firstname:  ['',[Validators.required, Validators.pattern('^[A-Za-z\\s]{3,15}$')]],
      lastname:  ['',[Validators.required, Validators.pattern('^[A-Za-z\\s]{3,15}$')]],
      businessname: ['',[Validators.required, Validators.pattern('^[A-Za-z0-9-_@.\\s]{4,30}$')]],
      dob: ['',Validators.required],
      location: ['',Validators.required],
      email: ['',[Validators.required, Validators.pattern('^[a-z0-9._]+@[a-z]{2,8}\.[a-z]{2,4}$')]],
      website: ['',[Validators.required, Validators.pattern('^(www.)?[a-z0-9/_]{2,50}?(.com)$')]],
      businessoverview: ['',[Validators.required, Validators.pattern('^[A-Za-z0-9._-\\s]+$')]],
      validity:  ['',[Validators.required, Validators.pattern('^[A-Za-z0-9]{1,2}$')]],
      agentname: ['',[Validators.required, Validators.pattern('^[A-Za-z\\s]{3,15}$')]],
